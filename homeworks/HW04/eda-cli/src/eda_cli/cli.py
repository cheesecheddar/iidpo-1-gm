...
def report(
    ...
    min_missing_share: float = typer.Option(0.2, help="Порог доли пропусков, выше которого колонка считается проблемной."),
) -> None:
    """
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write(f"Настройки отчёта: max_hist_columns={max_hist_columns}, top_k_categories={top_k_categories}, min_missing_share={min_missing_share:.2f}\n\n")
...
def head(
    path: str = typer.Argument(..., help="Путь к CSV-файлу."),
    n: int = typer.Option(5, help="Количество строк для отображения."),
    sep: str = typer.Option(",", help="Разделитель в CSV."),
    encoding: str = typer.Option("utf-8", help="Кодировка файла."),
) -> None:
    """
    Вывести первые несколько (5) строк датасета.
    """
    df = pd.read_csv(path, sep=sep, encoding=encoding)
    typer.echo(df.head(n))

    typer.echo(f"Первые {n} строк из {len(df)}")
    typer.echo(f"Файл: {path} (разделитель: '{sep}', кодировка: '{encoding}')")
