...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # есть ли колонки, где все значения одинаковы
    flags["has_constant_columns"] = any(col.unique == 1 for col in summary.columns)
    # есть ли категориальные признаки с очень большим числом уникальных значений
    flags["has_high_cardinality_categoricals"] = any(
        (not col.is_numeric and col.non_null > 0) and 
        (col.unique > 100 or 
        (summary.n_rows > 0 and col.unique / summary.n_rows > 0.5))
        for col in summary.columns
    )
    # есть ли уникальный индефикатор(user_id) ; при наличии дубликатов выставляется флаг
    flags["has_suspicious_id_duplicates"] = any(
        col.name.lower().endswith("id")
        and col.unique < summary.n_rows
        for col in summary.columns
    )
    # есть ли много нулевых значений в числовых колонках
    zero_cols: List[str] = []
    for col in summary.columns:
        if col.is_numeric and col.non_null > 0:
            ...
            # есть нули и положительные значения, mean << max (много нулей)
            elif col.min == 0 and col.max != 0 and col.mean is not None and col.mean <= 0.3 * col.max:
                zero_cols.append(col.name)

    flags["zero_value_columns"] = zero_cols
